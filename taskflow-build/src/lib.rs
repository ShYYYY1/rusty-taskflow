...
fn render_flow_builder(func_name: &str, nodes: &[Node]) -> Result<String, String> {
    ...
    body.push_str(&format!(
        "fn {func_name}() -> (rusty_taskflow::tf::flow::Flow, rusty_taskflow::tf::flow::TaskId) {{\n    let mut flow = rusty_taskflow::tf::flow::Flow::new();\n"
    ));
...
fn render_flow_runner(func_name: &str, nodes: &[Node]) -> Result<String, String> {
    ...
    let mut body = String::new();
    body.push_str(&format!(
        "async fn run_{func_name}() -> Result<std::sync::Arc<dyn std::any::Any + Send + Sync>, rusty_taskflow::tf::errors::FlowError> {{\n    let mut flow = rusty_taskflow::tf::flow::Flow::new();\n"
    ));
...
\n\
pub fn build_flow_by_path(path: &str) -> Option<(rusty_taskflow::tf::flow::Flow, rusty_taskflow::tf::flow::TaskId)> {{\n    match path {{\n{}\n        _ => None,\n    }}\n}}\n\
\n\
pub async fn run_flow_by_path(path: &str) -> Result<std::sync::Arc<dyn std::any::Any + Send + Sync>, rusty_taskflow::tf::errors::FlowError> {{\n    match path {{\n{}\n        _ => Err(rusty_taskflow::tf::errors::FlowError::ConfigBuildError(format!(\"flow path '{{}}' is not generated\", path))),\n    }}\n}}\n\n{}\n\n{}\n",
        path_entries.join(",\n"),
        match_arms.join("\n"),
        run_match_arms.join("\n"),
        builders.join("\n"),
